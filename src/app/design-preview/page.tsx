...
export default async function DesignPreviewPage({
  ...
  if (preview.view === "culture") {
    ...
    return cultureModule ? (
      <CultureModuleContent cultureModule={translateDeep(locale, cultureModule)} />
    ) : (
      <CultureTrack />
    );
  }

  const vocab = translateDeep(locale, [
  ...
  return (
    <LessonFlow
      unitNum={11}
      pathCode="A1.11"
      themeEn={tr(locale, "Greetings and farewells")}
      communicativeGoal={tr(locale, "Greet someone, respond politely, and say goodbye.")}
      cefrDescriptor={tr(locale, "Use a short, everyday exchange with familiar words.")}
      capstoneTask={tr(locale, "Complete a polite greeting and farewell.")}
      targetBand="A1"
      vocab={vocab}
      dialogues={translateDeep(locale, [
