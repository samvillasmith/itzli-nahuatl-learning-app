      ...
      <div className="border-b border-emerald-200/80 bg-[linear-gradient(100deg,#ecfdf5_0%,#f0fdfa_55%,#fffbeb_100%)] px-4 py-2.5 text-xs text-emerald-950">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-4">
          <p className="leading-relaxed sm:text-[13px]">
            ...
            <span className="font-semibold">{translate("A sweeping course-wide upgrade is here:")}</span>{" "}
            {translate("a sharper Eastern Huasteca focus, clearer lessons, reviewed vocabulary, richer practice, and a beautifully redesigned experience.")}{" "}
            <button
            ...
            >
              {translate("See what changed")}
            </button>
          </p>
          <button
            onClick={dismiss}
            aria-label={translate("Dismiss Version 2.0 announcement")}
        ...
        >
          ...
          >
            ...
            <div className="mb-6 inline-flex rounded-full bg-emerald-100 px-3 py-1 text-[10px] font-black uppercase tracking-[0.16em] text-emerald-800">
              {translate("Now available")}
            </div>
            <h2 className="pr-10 text-3xl font-black tracking-[-0.035em] text-stone-950">{translate("Welcome to Itzli 2.0")}</h2>
            <p className="mt-2 text-sm font-semibold text-emerald-800">{translate("A major course-wide upgrade")}</p>
            <p className="mt-4 text-sm leading-6 text-stone-600">
              {translate("We rebuilt Itzli to make learning Eastern Huasteca Nahuatl clearer, more focused, and more enjoyable from the first sound to the final unit.")}
            </p>
            ...
            <div className="mt-7 grid gap-3 text-sm text-stone-600 sm:grid-cols-2">
              <section className="rounded-2xl border border-emerald-100 bg-emerald-50/70 p-4">
                ...
                <h3 className="font-black text-stone-900">{translate("Eastern Huasteca, clearly")}</h3>
                <p>
                  {translate("The course now consistently centers the living variety spoken in and around Chicontepec, Veracruz.")}
                </p>
              </section>

              <section className="rounded-2xl border border-amber-100 bg-amber-50/70 p-4">
                <div className="mb-3 text-lg">◎</div>
                <h3 className="font-black text-stone-900">{translate("A stronger course")}</h3>
                <p>
                  {translate("A broad accuracy review corrected many mistranslations, damaged forms, and confusing lesson entries.")}
                </p>
              </section>

              <section className="rounded-2xl border border-sky-100 bg-sky-50/70 p-4">
                <div className="mb-3 text-lg">→</div>
                <h3 className="font-black text-stone-900">{translate("A smoother path")}</h3>
                <p>
                  {translate("Lessons, navigation, progress, and practice now work together as one guided learning experience.")}
                </p>
              </section>

              <section className="rounded-2xl border border-violet-100 bg-violet-50/70 p-4">
                <div className="mb-3 text-lg">◈</div>
                <h3 className="font-black text-stone-900">{translate("Made to be memorable")}</h3>
                <p>
                  {translate("A redesigned interface and richer visual vocabulary make every session calmer, clearer, and more inviting.")}
                </p>
              </section>
            </div>

            <div className="mt-5 rounded-2xl border border-stone-200 bg-white/80 p-4 text-xs leading-5 text-stone-600">
              <strong className="text-stone-900">{translate("Keeping Itzli sustainable:")}</strong>{" "}
              {translate("the paid AI tutor is paused. All lessons, grammar, vocabulary, audio, and practice activities remain available. Pronunciation audio is machine-generated educational support and may not reproduce every community pronunciation perfectly.")}
            </div>
            ...
            >
              {translate("Start exploring Version 2.0")}
            </button>
          </div>
        </div>
      )}
