        ...
        {(["A1", "A2", "B1"] as const).map((band) => {
          ...
          return (
            <div key={band} className="surface flex items-center gap-4 p-4">
              <span className={`flex h-11 w-11 items-center justify-center rounded-xl text-sm font-black ${band === "A1" ? "bg-emerald-100 text-emerald-800" : band === "A2" ? "bg-sky-100 text-sky-800" : "bg-violet-100 text-violet-800"}`}>{band}</span>
              <div><p className="font-black text-stone-950">{bandUnits.length} units</p><p className="text-xs text-stone-500">{words.toLocaleString()} learning words</p></div>
            </div>
          );
        })}
      </div>
